...
Use [Conventional Commits](https://www.conventionalcommits.org/) para manter histórico claro:

### Tipos Principais de Commits
- `feat`: Adiciona uma nova tarefa ou funcionalidade
- `fix`: Corrige um erro em uma tarefa existente
- `docs`: Atualiza documentação ou instruções
- `style`: Modifica estilo ou layout sem alterar funcionalidade
- `refactor`: Reestrutura código ou conteúdo sem alterar comportamento
- `chore`: Tarefas de manutenção ou atualizações de dependências
- `test`: Adiciona ou atualiza testes para tarefas ou funcionalidades
